...
// 查找项目目录（需求编号目录）
function findProjectDir(cwd, reqId) {
    if (reqId) {
    ...
    try {
        ...
        if (dirs.length === 1)
            return path.join(cwd, dirs[0].name);
        if (dirs.length > 1)
            return path.join(cwd, dirs[0].name); // 取第一个
    }
    catch { }
    return null;
}
// 加载 service-info.json
function loadServiceInfo(projectDir) {
...
function saveServiceInfo(projectDir, info) {
    const file = path.join(projectDir, "service-info.json");
...
// 保存 feedback.md
function saveFeedback(projectDir, items) {
    const file = path.join(projectDir, "feedback.md");
    ...
    for (const item of items) {
        ...
        if (item.needed)
            content += `**needed**: ${item.needed}\n`;
        content += `**created_at**: ${item.created_at}\n`;
        if (item.resolved_at)
            content += `**resolved_at**: ${item.resolved_at}\n`;
        if (item.resolution)
            content += `**resolution**: ${item.resolution}\n`;
    }
    fs.writeFileSync(file, content, "utf-8");
}
// 追加到 closed.md
function appendClosed(projectDir, item) {
...
// 创建初始目录结构
function createProjectStructure(projectDir, reqId) {
    const dirs = [
...
// 工具/命令执行函数
// ============================================================
async function handleCoordStart(params, cwd) {
...
async function handleCoordStatus(params, cwd) {
...
async function handleCoordPoll(params, cwd) {
    const projectDir = findProjectDir(cwd);
    if (!projectDir) {
    ...
    for (const issue of openIssues) {
        ...
        if (issue.needed)
            result += `  needed: ${issue.needed}\n`;
        result += `\n`;
    }
    return result;
}
async function handleCoordTestStart(params, cwd) {
    const projectDir = findProjectDir(cwd);
    if (!projectDir) {
...
async function handleCoordResolve(params, cwd) {
    const projectDir = findProjectDir(cwd);
    if (!projectDir) {
...
async function handleCoordDone(params, cwd) {
    const projectDir = findProjectDir(cwd);
    if (!projectDir) {
...
async function handleCoordBackendConfig(params, cwd) {
    const projectDir = findProjectDir(cwd);
    if (!projectDir) {
    ...
    if (params.action === "set" && params.field && params.value !== undefined) {
        ...
        if (params.field in serviceInfo.backend) {
            serviceInfo.backend[params.field] = value;
        }
        else {
...
export default definePluginEntry({
    ...
    register(api) {
        const toolOptions = { optional: false };
        ...
        api.registerTool({
            ...
            async execute(_id, params) {
                const result = await handleCoordStart(params, process.cwd());
        ...
        api.registerTool({
            ...
            async execute(_id, params) {
                const result = await handleCoordResolve(params, process.cwd());
                return { content: [{ type: "text", text: result }] };
            },
        }, toolOptions);
        api.registerTool({
        ...
        api.registerTool({
            ...
            async execute(_id, params) {
                const result = await handleCoordBackendConfig(params, process.cwd());
                return { content: [{ type: "text", text: result }] };
            },
        }, toolOptions);
